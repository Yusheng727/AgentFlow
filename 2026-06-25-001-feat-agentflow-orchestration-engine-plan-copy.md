...
- **R21.** **API 鉴权**：所有 REST 端点（`POST /workflows`、`GET /workflows/{id}/status`、`POST /workflows/{id}/retry`、`GET /trace`、`GET /diagnosis`、`GET /version-check`）必须经过鉴权。v1 采用简单 API Key + per-workflow ownership 校验（workflow_id 关联创建者，仅创建者可查 trace/诊断）。防止 IDOR——任意调用方枚举 workflow_id 读取他人 trace（含 LLM prompt、输出、业务数据）。
...
- **Virtual Threads + PG 并发写入策略（v3 新增）:**
  ...
  - **flush-then-barrier 不变式（v4.2 新增，修复 R3 冲突）**：barrier merge 计算 `channel_values` 前，必须先 `drainTo` + 同步 flush 异步队列，保证本 super-step 所有节点级输出已落盘；否则崩溃恢复会漏掉已 COMPLETED 但未 flush 的节点，导致重执行与 LLM 重复计费，违背 R3。批量 flush 仅在 super-step 内部用于摊销 DB 写入。
...
- **From-scratch rebuild premise doesn't answer "why not extend LangGraph4j"** — Problem Frame / Interview Value (P1, product-lens + adversarial, confidence 100)
...
- **Per-workflow ownership has no data store in zero-infra (InMemory) mode** — U14 / R18① / R21 (P1, security-lens, confidence 100)
...
- **No API Key registry, issuance, or validation source defined** — R21 / U14 (P1, security-lens, confidence 100)
...
- **R20 agentflow-archetypes has no implementation unit delivering it** — Requirements / U13 (P1, scope-guardian, confidence 100)
...
- **"七三开 (后端70%+Agent30%)" split not borne by unit inspection** — Interview Value / Problem Frame (P2, product-lens, confidence 75)
...
- **U14 (Week 5-6) depends on U7 (Week 6), start-before-dependency-ready** — Implementation Units / U14 (P2, scope-guardian, confidence 75)
...
- **Reducer abstraction built but no demo exercises channel conflict** — KTD-1 / U2 / R15-R17 (P2, adversarial, confidence 75)
...
- **KTD-1 BSP rationale defeats a strawman, not Actor/CSP** — KTD-1 (P2, adversarial, confidence 75)
...
- **Async node-level flush leaves a pre-barrier crash window defeating R3** — U5 / R3 / KTD-3 (P1, adversarial, confidence 75)
...
- **U13 (P0) depends on U12 (P1); Go/No-Go cut orphans P0 acceptance** — Implementation Units / U13 + Risks (P1, scope-guardian, confidence 75)
...
- **No tool-level authorization on YAML-submitted workflows** — R21 / U14 / KTD-7 (P1, security-lens, confidence 75)
...
- **Cost-risk "90% dev time mock" contradicts U9's Week 7 P1 schedule** — Risks / U9 (P2, product-lens, confidence 75)
...
- **Spring AI 2.1 risk mitigation cites v1.1 "distributed" as v1 differentiator** — Risks (P2, product-lens, confidence 75)
...
- **No per-request DAG size or token budget bound on POST /workflows** — U14 / R21 / R10 (P2, security-lens, confidence 75)
...
- **R10 "budget 告警" has no threshold source; budget_exceeded counter can't fire** — R10 / U7 (P2, scope-guardian, confidence 75)
...
- **R14 promises old-version execution but no store holds the old YAML definition** — R14 / U8 (P2, adversarial, confidence 75)
...
- **cancel() defaults to noop, so timeout/cost-control rests on an unenforced agent contract** — KTD-6 / U4 (P2, adversarial, confidence 75)
...
- **Agent 30% @Tool work overlaps InterviewCoach's Tool Calling claim** — Interview Value / Problem Frame (P2, product-lens, confidence 75)

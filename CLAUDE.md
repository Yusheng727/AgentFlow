...
**当前单元**：**U3 Agent 适配器层**（Week 3-4，P0）。OQ-2 已解决（Spring AI 2.0 GA 可拉，**Boot 升到 4.1.0**，非 plan 猜的 3.5）。KTD-7 冒烟已通过（`SpringAiApiSmokeTest` 3 tests，Advisor Chain + @Tool + usage 捕获 API 已验证）。按 plan：SpringAiAgentAdapter + Advisor Chain（TokenCountingAdvisor/LoggingAdvisor）+ OutputSchemaValidator + NodeRegistry + ExecutionTrace。**含 ce-code-review 留的 seam**：AgentInput 加 tools/outputSchema 字段、AgentOutput 加 structuredOutput + metadata（token 统计）。

> **U3 详细接手清单**：[`docs/handoff/u3-spring-ai-adapter.md`](docs/handoff/u3-spring-ai-adapter.md) — 下一个 Claude Code 会话在 `feat/u3-agent-adapter` 分支（`c406e9f`，已推 origin）上从该清单动手：已验证的 Spring AI 2.0 API 表面、要建的 8 个文件、执行顺序、开放项。
